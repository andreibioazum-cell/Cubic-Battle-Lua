...
local function loadPromoCodes()
    ...
    -- Загружаем использованные коды игроком
...
function lobby.draw()
    ...
    love.graphics.printf("Cubicoins: " .. (coins or 0), 0, h / 2 - 80, w, "center")
    ...
    if promoActive then
        ...
        if displayText == "" then
            ...
            love.graphics.printf("Enter code...", inputX + 10, inputY + 10, inputW - 20, "left")
        else
            love.graphics.setColor(1, 1, 0)
        ...
        if x >= item_x + item_w - 80 and x <= item_x + item_w - 20 and
           y >= item_y + 15 and y <= item_y + 45 then
            ...
            if not skins.diamond.owned and coins >= 100 then
                ...
                saveGame()
                print("Diamond bought! Coins: " .. coins)
            elseif skins.diamond.owned then
                selected_skin = "diamond"
                saveGame()
                print("Diamond equipped!")
            else
                print("Not enough coins! Need 100, have " .. coins)
            end
            return
        end
    end
