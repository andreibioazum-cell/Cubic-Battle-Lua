...
function game.update(dt)
    ...
    for i=#bullets, 1, -1 do
        ...
        if b.x < 0 or b.x > WORLD_SIZE or b.y < 0 or b.y > WORLD_SIZE then table.remove(bullets, i) end
    end

    enemy.update(dt, cube.x, cube.y, bullets, function(dmg)
        cube.hp = cube.hp - dmg
        if cube.hp <= 0 then dead = true; if game.onDeath then game.onDeath() end end
    end)
end
...
function game.draw()
    ...
    love.graphics.draw(img, cube.x, cube.y, cube.angle, 55/img:getWidth(), 55/img:getHeight(), img:getWidth()/2, img:getHeight()/2)
...
function game.setOnDeath(fn) game.onDeath = fn end
function game.setCoins(c) coins = c end
function game.setSkin(s) selected_skin = s end
